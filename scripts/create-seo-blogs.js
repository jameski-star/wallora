...
// Define the 4 blog topics with their metadata and filtering criteria
const BLOG_TOPICS = [
  {
    topic: "Best Sites to Find and Download 4K Wallpapers for Every Screen",
    keywords: ["wallpapers in 4k", "wallpapers 4k"],
    categoryFilter: (c) => ['nature', 'minimal', 'space', 'dark', 'abstract'].includes(c.slug),
    wallpaperFilter: (w) => w.resolution === '3840x2160' || ['nature', 'minimal', 'space', 'dark'].includes(w.category_slug),
    limit: 15
  },
  {
    topic: "Top Desktop Wallpaper Sites to Elevate Your Workspace Setup",
    keywords: ["wallpapers for computer", "wallpapers computer"],
    categoryFilter: (c) => ['gaming', 'minimal', 'technology', 'cities', 'sci-fi'].includes(c.slug),
    wallpaperFilter: (w) => w.device === 'desktop',
    limit: 15
  },
  {
    topic: "Where to Find the Best iPhone & Android Wallpapers in 2026",
    keywords: ["wallpapers for phone", "wallpapers iphone"],
    categoryFilter: (c) => ['minimal', 'dark', 'illustration', 'anime', 'nature'].includes(c.slug),
    wallpaperFilter: (w) => w.device === 'phone',
    limit: 15
  },
  {
...
async function generateBlog(topicInfo, categories, allWallpapers) {
  console.log(`\nGenerating blog for topic: "${topicInfo.topic}"...`);
  
  // Filter relevant categories and wallpapers
  ...
  const prompt = [
    ...
    "3. Interlinking & Backlinks: Provide clear, HTML/Markdown links pointing to the Aurava site. You must include links such as [Aurava](/) (homepage), [Aurava's wallpaper collection](/wallpapers), or category pages like [Nature Wallpapers](/wallpapers/nature), [Minimalist Wallpapers](/wallpapers/minimal), [Anime Wallpapers](/wallpapers/anime), [Space Wallpapers](/wallpapers/space) depending on the topic context. Ensure there are at least 3-4 links to Aurava throughout the article.",
    ...
    "   - **Download:** [Download on Aurava](/wallpapers/slug)",
    "",
...
async function run() {
  try {
    // 1. Fetch categories and wallpapers
    ...
    console.log(`Fetched ${categories.length} categories and ${allWallpapers.length} wallpapers.`);
    
    const generatedPosts = [];
    
    // 2. Generate each blog post
    for (const topicInfo of BLOG_TOPICS) {
      try {
        ...
        // 3. Upsert to DB
        // database fields: id, slug, title, excerpt, body, cover_image, author, tags, published, seo_title, seo_description, created_at, updated_at
        const row = {
          id: post.id,
          slug: post.slug,
          title: post.title,
        ...
        console.error(`Failed to generate/save blog for topic: ${topicInfo.topic}`, err);
      }
    }
    
    console.log(`\nAll blog creation finished. Total successfully processed: ${generatedPosts.length}`);
  } catch (err) {
